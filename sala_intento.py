...
class Bullet():
    def __init__(self, NumP, position, direction, id,game, speed = 30):
        self.game = game
        self.owner = NumP
        self.id = id
        #self.width = BullSize
    ...
    def update(self):
        if self.dir == 0:
            if not self.collide_with_walls(self,-self.speed,0):
                self.pos[0] -= self.speed
            else:
                self.game.elimbull(self)

        elif self.dir == 1:
            if not self.collide_with_walls(self,0,-self.speed):
...
class Game():
    ...
    def shoot(self, numP):
        ...
        if dir == 0:
            pos[0] = pos[0] - 45
            bullet = Bullet(owner, pos, dir, id,self)
